...
code_to_run = <<-EOH
          web_api_key=`mysql --skip-column-names -h #{node[:app][:zend_server_mysql_address]} -u#{node[:app][:zend_server_mysql_user]} -p#{node[:app][:zend_server_mysql_password]} ZendServer << EOF
